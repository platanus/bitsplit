class Api::V1::PaymentsController < ApplicationController
    ...
    def create
        ...
        #setear esta variable para simular o no
        ...
        if @receiver_user != nil
            if @user.api_key!= nil && @receiver_user.api_key!= nil
                ...
                if @user.logged and @receiver_user.logged
                    invoice=create_new_invoice(bitcoins_amount, receiver_user_api_key, receiver_user_api_secret)
                    if invoice.has_key? "invoice"
                        invoice_body=JSON.parse(invoice.body)
                        invoice_code=invoice_body["invoice"]["encoded_payment_request"]
                        invoice_payment = pay_new_invoice(bitcoins_amount, invoice_code, payment_simulation, user_api_key, user_api_secret)
                    ...
                    @error_message = "both users must be logged in" 
                    render "error"
                end
            else
